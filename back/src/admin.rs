...
pub async fn index(_req: Request) -> Resp {
	let mut html = fs::File::open("../front/templates/admin/index.html").unwrap();
	let mut main_buf: Vec<u8> = Vec::new();
	html.read_to_end(&mut main_buf).unwrap();
	let mut tpls_buf: Vec<u8> = Vec::new();
	let dirs = vec!["core", "admin"];
	for dir in dirs {
		let path = "../front/templates/".to_owned() + dir;
		for fname in fs::read_dir(path).unwrap()
